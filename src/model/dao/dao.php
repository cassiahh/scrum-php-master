...
class TarefaDao
{
    ...
    function listaTarefas()
    {
        ...
        $resultado = mysqli_query($this->conexao, "select t.*, p.nome, p.papel
                from tarefa as t join pessoa as p on t.idPessoa=p.idPessoa ");
    ...
    {
        $query = "insert into tarefas (idSprint, tarefa, idPessoa, status,
            inicio, termino, objetivo)
            values ('{$tarefa->getIdSprint()}', {$tarefa->getTarefa()},
            '{$tarefa->getPessoa()->getIdPessoa()}',
            {$tarefa->getStatus()},
    ...
    function alteraTarefa(Tarefa $tarefa)
    {
        $query = "update tarefas set idSprint = '{$tarefa->getTarefa()}',
            tarefa = {$tarefa->getTarefa()}, idPessoa = '{$tarefa->getPessoa()->getIdPessoa()}',
